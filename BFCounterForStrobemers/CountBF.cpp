...
struct CountBF_ProgramOptions {
  ...
  CountBF_ProgramOptions() : k(0), nkmers(0), verbose(false), quake(false), bf(4), qs(0), threads(1), seed(0), min_window(10), min_window_offset(20), min_num(5), min_len(5), read_chunksize (10000) {}
};
...
void CountBF_PrintUsage() {
  ...
  cerr << endl << endl <<
    ...
    "-w, --min-window=INT            Minimizer window length (default 10)" << endl <<
    "-f, --min-window-offset=INT     Minimizer window offset (default 20)" << endl <<
...
void CountBF_ParseOptions(int argc, char **argv, CountBF_ProgramOptions &opt) {
  int verbose_flag = 0;
  int quake_flag = 0;
  const char* opt_string = "n:k:o:b:t:s:c:w:m:l:f:";
  static struct option long_options[] =
  {
    {"verbose", no_argument,  &verbose_flag, 1},
    {"kmer-size", required_argument, 0, 'k'},
    {"min-window", required_argument, 0, 'w'},
    {"min-window-offset", required_argument, 0, 'f'},
  ...
  while (true) {
    ...
    case 'w': 
      ...
      break;
    case 'f': 
      opt.min_window_offset = atoi(optarg); 
      break;
    case 'm': 
...
void CountBF_Normal(const CountBF_ProgramOptions &opt) {
  ...
  int min_window_offset = opt.min_window_offset; // the position of the minimizer window from previous minimizer window
  ...
  //char name[8196],s[8196];//, qual[8196];
  char name[8196],s[800000];//, qual[8196];
...
#pragma omp for nowait
      ...
      // for each read in our batch
        ...
        iter = KmerIterator(cstr, min_window, min_num, min_len, min_window_offset);
        ...
        // for each valid k-mer in read
          ++num_kmers;
          //Kmer rep = iter->first.rep(); NO REP
          Kmer rep = iter->first;
