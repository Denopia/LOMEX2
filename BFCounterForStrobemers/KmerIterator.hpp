...
public:
  ...
  KmerIterator(const char* s, int mw, int mn, int ml, int mwf) : s_(s), p_(), invalid_(false), min_window_(mw), min_num_(mn), min_len_(ml), min_window_offset_(mwf) 
  {
    p_.second = 0;
...
private:
  ...
  std::map<int, std::vector<std::tuple<uint64_t,int,int> > > minimizers_faster_super_reverse_; // map with minimizer window position as key, value is tuple with [hash value, position, strand(0=forward, 1=reverse)]
  ...
  unsigned int min_window_offset_;
