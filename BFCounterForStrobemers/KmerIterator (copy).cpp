...
void KmerIterator::find_minimizers_faster_super()
{
  ...
  for (int i = 0; i < min_window_; i+=1)
  {
    ...
    if(f<r){kmer_queue.push_back(std::make_tuple(1,0,current_position,f));}
    else if(f>r){kmer_queue.push_back(std::make_tuple(1,1,current_position,r));}
    else{kmer_queue.push_back(std::make_tuple(0,0,current_position,f));}
  ...
  if (minimizer_found)
  {
    minimizers_faster_super_[0].push_back(std::make_tuple(minimum_in_window,current_min_position,current_orientation));
    //std::cout << "Minimizer for window in position 0 is in position " << current_min_position << " and its hash value is " << minimum_in_window << " with orientation " << current_orientation << "\n"; 
  }
  ...
  while (window_start +  min_window_ - 1 + min_len_ - 1 < read_len_)
  {
    ...
    if (minimizer_found && current_min_position >= window_start)
    {
      minimizers_faster_super_[window_start].push_back(std::make_tuple(minimum_in_window,current_min_position,current_orientation));
      //std::cout << "Minimizer for window in position "<< window_start << " is in position " << current_min_position << " and its hash value is " << minimum_in_window << " with orientation " << current_orientation << "\n";
      //std::cout << "NO NEED TO RE-CALC\n";
    }

    if (current_min_position < window_start) // Calculate window minimum again
    {
      uint64_t minimum_in_window = std::numeric_limits<uint64_t>::max(); // keeps track of the minimum k-mer in the current window
      int current_min_position = 0;
      int current_orientation = 0;
      ...
      for (int apo = 0; apo < kmer_queue.size(); apo+=1)
      {
        ...
        if (std::get<3>(entry) <= minimum_in_window)
        {
...
bool KmerIterator::find_next_III()
{
  ...
  //int previous_minimizer_end_position = pos-1;

  for (int i = 0; i < min_num_; i+=1) // The number of minimizers needed for a spaced k-mer
  {
    //std::cout << "\n";
    //std::cout << "MIN NUMBER " << i << " OUT OF " << min_num_ << "\n";
   
    int min_pos = p_.second + i*(min_len_+min_window_-1); // position of the next minimizer
    
    //int min_pos = previous_minimizer_end_position+1;


    //std::cout << "WINDOW POSITION " << min_pos << "\n";  
    //std::cout << "POSITION IS " << min_pos << " AND READ LENGTH IS " << read_len_ << "\n"; 
    ...
    //std::cout << "VALID\n";
    //if (minimizers_.count(min_pos) < 1)
    if (minimizers_faster_super_.count(min_pos) < 1)
    {
      //std::cout << "DID NOT FIND MINIMIZER FOR POSITION\n";
      return false;    
      // return with no spaced k-mer for this starting pos argument
    }
    //std::cout << "FOUND MINIMIZER FOR POSITION\n";
    //fixed_streak_starts.push_back(std::get<1>(minimizers_[min_pos][0])); // minimizer position -> [0] one of the minimizers of the window -> [1] minimizer position in read
    int position_tmp = std::get<1>(minimizers_faster_super_[min_pos][0]);
    int orientation_tmp = std::get<2>(minimizers_faster_super_[min_pos][0]);

    //std::cout << "MINIMIZER FOR THIS WINDOW IS IN POSITION " << position_tmp << "\n";

    fixed_streak_starts.push_back(std::make_tuple(orientation_tmp, position_tmp));

    //previous_minimizer_end_position = position_tmp + min_len_-1;
  ...
  /*
      Check that the k-mer is not the same as previous one
  */

  /*
  if (minimizer_kmer_fixed_streak_starts_.size() != 0)
  {
    bool same_as_previous_kmer = true;
    for (int i = 0; i < min_num_; i+=1)
    {
      if (std::get<1>(fixed_streak_starts[i]) != std::get<1>(minimizer_kmer_fixed_streak_starts_[i]))
      {
        same_as_previous_kmer = false;
        break;
      }
    }
    if (same_as_previous_kmer)
    {
      //std::cout << "SKIPPED K-MER\n";
      //std::cout << "THIS POSITION HAS THE SAME K-MER AS ABOVE\n";
      return false;
    }
    else 
    {
      minimizer_kmer_fixed_streak_starts_ = fixed_streak_starts;
    }
  }
  else
  {
  ...
  */
  ...
  /*
    ...
    Following stuff is modified so the concatenations make sense

  */

  // Build spaced k-mer by concatenating fixed streaks

  char minstring[min_len_*min_num_+1];
  char minstring_rev[min_len_*min_num_+1];

  //int msp = 0; // minimizer string position
  
  int fmwp = 0; // forward minimizer window position
  //int bmwp = min_num_*min_len_ - min_len_; // backward minimizer window position
  int bmwp = min_num_*min_len_ - 1;
  ...
  //for (int x = 0; x < min_len_*min_num_; x+=1)
  //{
  //  minstring_rev[min_len_*min_num_-1-x] = reverse_complement_character(minstring[x])
  //} 

  minstring[min_len_*min_num_] = '\0';
  //minstring_rev[min_len_*min_num] = '\0';

  /*
    if (mo==0) // forward stuff
    {
      for (int x = 0; x < min_len_; x+=1)
      {
        minstring[fmwp+mcp] = s_[mp+x];
        minstring_rev[bmwp+mcp] = s_[mp+x];
        mcp+=1;
      }
      mcp = 0;
      fmwp = fmwp + min_len_;
    ...
    else if (mo==1)
    {
      ...
      {
        int y = min_len_ - 1 - x;
        minstring[fmwp+mcp] = reverse_complement_character(s_[mp+y]);
        minstring_rev[bmwp+mcp] = reverse_complement_character(s_[mp+y]);
        mcp+=1;
      }
      mcp = 0;
      fmwp = fmwp + min_len_;
  ...
  std::string ks = next_kmer_candidate1.rep().toString();



  // BLOCK BELOW: Discard k-mer if it was found in the PREVIOUS position of the read

  if (previous_minimizer_kmer_ == ks)
  {
    return false;
  }
  else if (found_minimizer_kmers_.count(ks) != 0)
  {
    //std::cout << "NON-CONSECUTIVE DOUBLE\n";
    return false;
  }
  else
  {
    found_minimizer_kmers_[ks] = 1;
    previous_minimizer_kmer_ = ks;
    p_.first = next_kmer_candidate1;
    return true;
  }


  // BLOCK BELOW: Discard k-mer if it was found ANYWHERE earlier in the read

  /* 
  
  if (found_minimizer_kmers_.count(ks) == 0)
  {
    found_minimizer_kmers_[ks] = 1;
    p_.first = next_kmer_candidate1;
    return true;
  ...
  */
  ...
  // BELOW: some old garbage 

  //Kmer next_kmer_candidate2 = Kmer(minstring_rev);

  //if (next_kmer_candidate1.rep() < next_kmer_candidate2.rep())
  //{
  //  p_.first = next_kmer_candidate1;
  //}
  //else
  //{
  //  p_.first = next_kmer_candidate2;
  //}
