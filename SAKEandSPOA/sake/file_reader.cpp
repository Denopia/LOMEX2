	...
	while (std::getline(fastq_file, fastq_line, '\n')){
		...
		if (read_line == 1){
			...
			if (current_read_number > last){
				reads_left = false;
				kill_me();
				std::string message =  "My first read is " + std::to_string(first) + " and my last read is " + std::to_string(last) + "\n";
				std::cout << message;
				return;
			}
			if (current_read_number >= first){

				//std::string msg = "Read is " + std::to_string(current_read_number) + "\n";
				//std::cout << msg;
